...
### 2026-05-16：完成 V2-016 首访语法能力诊断

- 状态：已完成
- 来源：FB-013
- 内容：
  - 首页首次进入时弹出欢迎弹窗，标题为“欢迎来到小德英语lab的四六级语法网站”。
  - 弹窗提供两条路径：开始 8 题语法检测，或选择“我是语法小白，从第 1 关开始”。
  - 8 道诊断题覆盖句子主干、词性位置、从句功能、非谓语、逻辑关系、时态、长难句压缩和特殊结构。
  - 诊断结束后根据错题推荐薄弱关卡，最多展示 3 个推荐关卡，并自动解锁到最高推荐关卡。
  - 小白路径会把首页主按钮固定为“开始第 1 关”，避免已解锁进度干扰从零学习。
  - 首访弹窗使用 `localStorage` 记录是否已展示，避免每次刷新重复打扰用户；诊断流程只在弹窗内完成，不在首页嵌入诊断大板块。
  - `HomePage.test.tsx` 增加首访弹窗、小白路径、8 题诊断推荐和解锁逻辑回归测试。
- 验收：
  - TDD 红灯：`npm test -- HomePage` 初次失败，确认旧首页没有首访诊断弹窗和 8 题推荐流程。
...
### 2026-05-16：完成 V2-017 首访诊断题难度升级

- 状态：已完成
- 来源：FB-014
- 内容：
  - 将首访 8 道诊断题从基础短句升级为更接近四六级的长难句筛查题。
  - 主干题加入定语从句和时间状语干扰，避免只看最近名词/动词作答。
  - 词性题从简单名词前形容词升级为 `consider + object + complement` 结构。
  - 从句题改为 `The fact that... shows...`，区分同位语从句和宾语从句。
  - 非谓语题改为句首过去分词状语，考查非谓语不抢主线。
  - 逻辑题改为 `While..., ...` 转向判断，干扰项更像真实误判。
  - 时态题改为 `By the time... had collected`，考查过去完成时参照点。
  - 压缩题和特殊结构题分别升级为多层修饰主干压缩、`Not until... do...` 倒装理解。
  - `HomePage.test.tsx` 同步更新答题路径，继续锁住“只错逻辑题时推荐第 6 关”的推荐逻辑。
- 验收：
